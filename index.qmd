...
ggplot(selected_data, aes(x = admission_grade, y = first_year_grade)) +
  ...
  labs(
    title = "Relationship Between Squared Admission Grade and First-Year Grade",
    x = "Admission Grade",
    y = "First-Year Grade"
  ) +
  theme_minimal()
...
ggplot(selected_data, aes(x = admission_grade, y = previous_qualification_grade)) +
  ...
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(
    title = "Relationship Between Squared Admission Grade and Previous Qualification Grade",
    x = "Admission Grade",
...
**Grade Distributions**
- Most students have high grades, but there is noticeable variation.


**Correlations**
- Admission grades moderately correlate with semester and previous qualification grades, showing they influence performance but aren’t the only factor.

**Linear Relationships**
- Admission grades predict first-year grades (R² = `r r2_linear`), with the squared model (R² = `r r2_squared`) fitting slightly better.

 **Group Trends**
- **Scholarship Holders**: Perform better than non-holders.
- **Attendance Type**: Daytime students outperform evening students.
- **Gender**: No major differences.


- Admission grades are useful predictors, but other factors matter too.
- Scholarships and support for evening students could improve outcomes.

# Appendix

Lev Laptinov - Introduciton, Data Loading and Preprocessing, Research Question, Exploratory Data Analysis

Dmytro Kolisnyk - Methodology, Correlation, part of Linear Relationships

Vasyl Moiseiev - part of Linear Relationships, Discussion and Conclusion